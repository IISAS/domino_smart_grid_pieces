...
class ElectricityPricePredictionModelTrainPiece(BasePiece):
    ...
    def piece_function(self, input_data: InputModel):
        ...
        def _read_input_rows(raw_payload: dict) -> list[dict]:
            ...
            tabular_data = raw_payload.get("tabular_data") or raw_payload.get(
                "dataframe"
            )
        ...
        def _extract_row_datetime(
            row: dict, datetime_keys: tuple[str, ...]
        ) -> dt.datetime | None:
        ...
        def _extract_price_map(okte_rows: list[dict]) -> dict[tuple[str, str], float]:
            ...
            for row in okte_rows:
                ...
                hour_raw = (
                    row.get("hour") or row.get("delivery_hour") or row.get("slot")
                )
        ...
        def _enrich_rows_with_okte(
            ...
            all_prices = list(direct_price_map.values()) + list(
                fallback_price_map.values()
            )
            ...
            for row, row_ts in zip(input_rows, row_datetimes):
                ...
                enriched["price_source"] = (
                    "okte_direct"
                    if (date_key, slot_key) in direct_price_map
                    else "fallback_28d_avg"
                )
        ...
        if not output_dir:
            ...
            output_dir = str(
                getattr(self, "results_path", None) or tempfile.gettempdir()
            )

        os.makedirs(output_dir, exist_ok=True)

        model_filename = str(
            payload.get("model_filename", "electricity_price_xgb.joblib")
        )
